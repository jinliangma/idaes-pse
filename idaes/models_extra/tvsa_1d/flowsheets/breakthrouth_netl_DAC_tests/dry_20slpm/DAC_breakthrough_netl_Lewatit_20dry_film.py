...
def get_model(dynamic=True, time_set=None, nstep=None, init=True):
    ...
    flow_mol_gas = 0.0681*0.2 # based on NETL test
...
def main_dynamic():
    ...
    for x in xpoint:
        ...
        for t in m_dyn.fs.config.time:
            ...
            y_co2_mf_film.append(value(m_dyn.fs.FB.mole_frac_comp_surface[t,x,"CO2"]))
            y_h2o_mf.append(value(m_dyn.fs.FB.gas_phase.properties[t,x].mole_frac_comp["H2O"]))
            y_h2o_mf_film.append(value(m_dyn.fs.FB.mole_frac_comp_surface[t,x,"H2O"]))
