...
# desorption step simulation using microwave heating
...
def get_model(dynamic=True, time_set=None, nstep=None, init=True):
    ...
    m.fs.FB.microwave_heating_rate_active.fix(0)
    m.fs.FB.microwave_heating_rate_inactive.fix(0)
    m.fs.FB.htc_between_solid_phases = 10 #W/K-kg original 10
...
def main_dynamic():
    ...
    for t in m_dyn.fs.time:
        if t>=30:
            m_dyn.fs.FB.microwave_heating_rate_active[t,:].fix(2e5) # originally 1e5
            m_dyn.fs.FB.microwave_heating_rate_inactive[t,:].fix(2e4) # originally 1e3
        ...
        if t>=210 and t<450:
            qactive = 2e5 -2e5/(450-210)*(t-210)
            qinactive = 2e4 -2e4/(450-210)*(t-210)
            m_dyn.fs.FB.microwave_heating_rate_active[t,:].fix(qactive)
            m_dyn.fs.FB.microwave_heating_rate_inactive[t,:].fix(qinactive)
        if t>=450:
            m_dyn.fs.FB.microwave_heating_rate_active[t,:].fix(0)
            m_dyn.fs.FB.microwave_heating_rate_inactive[t,:].fix(0)
    ...
    for x in xpoint:
        ...
        for t in m_dyn.fs.config.time:
            ...
            y_solid_temp_active.append(value(m_dyn.fs.FB.solid_temperature_active[t,x]-273.15))
            y_solid_temp_diff.append(value(m_dyn.fs.FB.solid_temperature_active[t,x]-m_dyn.fs.FB.solid_temperature[t,x]))
            ...
            y_heat_mw_inactive.append(value(m_dyn.fs.FB.microwave_heating_rate_inactive[t,x]*m_dyn.fs.FB.bed_area))
            y_heat_mw_active.append(value(m_dyn.fs.FB.microwave_heating_rate_active[t,x]*m_dyn.fs.FB.bed_area))
        solid_temp[xlabel[ix]] = y_solid_temp
        solid_temp_active[xlabel[ix]] = y_solid_temp_active
        solid_temp_diff[xlabel[ix]] = y_solid_temp_diff
        ...
        heat_mw_inactive[xlabel[ix]] = y_heat_mw_inactive
        heat_mw_active[xlabel[ix]] = y_heat_mw_active
        ix += 1

    solid_temp_df = pd.DataFrame(solid_temp)
    solid_temp_active_df = pd.DataFrame(solid_temp_active)
    solid_temp_diff_df = pd.DataFrame(solid_temp_diff)
    ...
    heat_mw_inactive_df = pd.DataFrame(heat_mw_inactive)
    heat_mw_active_df = pd.DataFrame(heat_mw_active)
    ...
    plt.figure(15)
    plt.plot(time, solid_temp_active_df, label=xlabel)
    plt.legend()
    plt.grid()
    plt.xlabel("Time [s]")
    plt.ylabel("Active Solid Temperature [C]")
    plt.show(block=False)

    plt.figure(16)
    plt.plot(time, solid_temp_diff_df, label=xlabel)
    plt.legend()
    plt.grid()
    plt.xlabel("Time [s]")
    plt.ylabel("Differnece Between Active and Inactive Temperatures [C]")
    plt.show(block=False)

    plt.figure(17)
    plt.plot(time, heat_mw_inactive_df, label=xlabel)
    plt.legend()
    plt.grid()
    plt.xlabel("Time [s]")
    plt.ylabel("Microwave heat duty inactive [W/m]")
    plt.show(block=False)

    plt.figure(18)
    plt.plot(time, heat_mw_active_df, label=xlabel)
    plt.legend()
    plt.grid()
    plt.xlabel("Time [s]")
    plt.ylabel("Microwave heat duty active [W/m]")
    plt.show(block=True)
...
def write_dynamic_results_to_csv(m_dyn,filename="dynamic_results.csv"):
    ...
    col = []
    col.append("time_s")
    var_heading = ("active_solid_temperature_C_x",
                   "inactive_solid_temperature_C_x",
                   "gas_tempreature_C_x",
                   ...
                   "active_microwave_heat_W/m_x",
                   "inactive_microwave_heat_W/m_x"
                   )
    ...
    for x in m_dyn.fs.FB.length_domain:
        ...
        for t in m_dyn.fs.time:
            ivar = 0
            data[irow,1+ix+len_x*ivar] = value(m_dyn.fs.FB.solid_temperature_active[t,x]-273.15)
            ivar = 1
            ...
            ivar = 15
            data[irow,1+ix+len_x*ivar] = value(m_dyn.fs.FB.microwave_heating_rate_active[t,x]*m_dyn.fs.FB.bed_area)
            ivar = 16
            data[irow,1+ix+len_x*ivar] = value(m_dyn.fs.FB.microwave_heating_rate_inactive[t,x]*m_dyn.fs.FB.bed_area)
            irow += 1
        ix += 1
    irow = 0
    for t in m_dyn.fs.time:
