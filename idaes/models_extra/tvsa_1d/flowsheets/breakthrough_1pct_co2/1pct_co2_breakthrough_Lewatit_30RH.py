...
def get_model(dynamic=True, time_set=None, nstep=None, init=True):
    ...
    m.fs.FB.kf["CO2"] = 0.003  # original 0.003 from Young's paper
    m.fs.FB.kf["H2O"] = 0.03  # original 0.0086 from Young's paper
    m.fs.FB.bed_diameter.fix(0.0067)
    m.fs.FB.wall_diameter.fix(0.0068)
    m.fs.FB.bed_height.fix(0.01074*1.08)
...
def main_dynamic():
    ...
    write_dynamic_results_to_csv(m_dyn, "Lewatit_1pct_dry_result_25C_humid.csv")
    # ------------------------------------------------------------------------------
    # plot figures
    time = []
